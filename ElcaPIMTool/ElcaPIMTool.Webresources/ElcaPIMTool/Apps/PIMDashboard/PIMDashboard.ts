...
window.deleteMultiProject = async function () {
  ...
  if (confirm(confirmMessage)) {
    ...
    class errorInfo {
      ...
      relatedId: String;
      relatedName: String;
      constructor(mes: String, relatedId: String, relatedName: String) {
    ...
    for (var i = 0; i < checkedList.length; i++) {
      let projectId = checkedList[i].getAttribute("id");
      let projectName = checkedList[i].getAttribute("value");
        //@ts-ignore
        try{
          ...
          if(!errorMessage.includes(err.message)){
            ...
            errorList.push(new errorInfo(err.message,projectId,projectName))
          }
          else{
